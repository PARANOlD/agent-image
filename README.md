...
- **Won't respond to everything**: an explicit `@mention` or a DM always gets a response. A plain threaded reply with no mention goes through `intent_gate.py`: his name appearing in the text is an instant yes (no model call), otherwise the model is asked, and it's given Gary's last message in that thread as context so it can tell a follow-up ("now add error handling") from unrelated chatter. Errors fail toward *answering* -- inside a thread he's already in, a stray reply costs less than silently ignoring a direct question.
...
- **Deterministic tools, LLM-routed**: concrete actions (PR status, listing open PRs, cutting a branch + opening a PR, opening a ticket) are plain Python (`github_actions.py`, `github_pr_creator.py`, `github_ticket_creator.py`) that make real API/git calls and can't hallucinate a result. Getting from a Slack sentence to "which tool, what parameters" goes through `llm_router.py` -- one narrow classification call, same shape as `intent_gate.py`'s proven yes/no pattern, just richer JSON output. The model never gets closer to *executing* anything than picking a name and a number; this is deliberate, see `openhands_client.py`'s `native_tool_calling` comment for why actual model-driven tool-calling was abandoned. Extend `GITHUB_COMMANDS` in `app.py` and add a handler function to add more. The prompt explicitly tells the model never to invent a required param it can't find in the message (e.g. a PR number) -- without that instruction it would occasionally hallucinate one rather than fall through to a command needing no params.
- **Cutting a branch + opening a PR** (`github_pr_creator.py`): `@Gary cut a feature branch that adds a CONTRIBUTING.md file` -- branch type is `feature`/`bugfix`/`hotfix` (defaults to `feature`), everything else comes from two separate model calls kept deliberately narrow: one small-JSON call for branch slug/commit message/PR title+body, one plain-text call for the actual file content (v1 is single-file only). Git operations are real `git` subprocess calls against a throwaway clone (needs `git` in the chat-bridge image -- see its Dockerfile). Opens the PR via the GitHub API, replies with the URL, and separately alerts `SLACK_STATUS_CHANNEL` so `#gary` hears about it even if the request came from a DM or another channel. **This command is for "cut a branch/open a PR right now" -- a general feature request should go to `create_ticket` instead**, see below.
- **Opening a ticket** (`github_ticket_creator.py`): `@Gary I'd like to add dark mode to the config page` -- a general change request opens a GitHub issue (one narrow model call for title/body) rather than immediately cutting code, so the AC can be discussed and refined first. Replies with the issue number and URL. This is step 1 of the ticket -> pick-up -> draft-PR -> collaborate -> merge workflow described in `OFFERING.md`; **picking up a ticket (branch + draft PR linked to the issue), pushing further collaborative commits to that branch, converting the draft PR to ready-for-review, and repo squash/delete-branch merge settings are not built yet.**
- **Starting work on a branch you already created**: `@Gary get started on feature/login-fix` -- looks the branch up via the GitHub API (no local git clone; nothing needs writing yet) and confirms it exists with its latest commit. The branch is remembered as this thread's active branch (`state.py`'s `active_branch` table) for future commands to read -- **but nothing yet pushes further commits to an existing branch**, so right now this is just a checked-and-tracked confirmation, not a working checkout.
- **Router commands work as same-thread follow-ups once a branch is tracked**: normally the router-command shortcut only runs on an explicit @mention/DM (`allow_new`), since it bypasses the intent gate. Once `start_work` has set an active branch for a thread, that thread's plain replies (no re-mention needed) are routed too -- an unambiguous signal the user is mid-workflow with Gary right there, same reasoning as `is_directed_at_gary`'s thread-context check for the OpenHands path. Without this, a same-thread follow-up like "go ahead and open a draft PR for this branch" went nowhere: router commands never register an OpenHands conversation, so the fallback path found nothing to continue and silently dropped it (caught 2026-09-16).
- **Opening a PR for an already-tracked branch, optionally as draft** (`open_pr_for_branch` in `github_pr_creator.py`): `@Gary go ahead and create a draft PR for this branch to main` (as a follow-up in a thread with an active branch, no re-mention needed) -- unlike `create_pr`, this opens a PR for a branch that already exists (no clone, no invented file, no new branch); title/body come from one narrow model call seeded with the branch name and its latest commit message. Defaults to draft; the router only sets `draft: false` when the message explicitly asks for that (worth a second look if you see it flip unprompted -- caught once already where the model defaulted to non-draft with nothing said either way, fixed by being explicit in the prompt about what "unset" should mean).
- **Marking an existing PR ready for review** (`mark_pr_ready` in `github_actions.py`): `@Gary set that to active` -- converts the tracked branch's PR from draft to ready. GitHub's REST "update pull request" endpoint silently ignores a `draft` field on PATCH (returns 200, doesn't actually change anything -- verified empirically 2026-09-16 before relying on it); this is GraphQL-only, via the `markPullRequestReadyForReview` mutation, which needs the PR's GraphQL `node_id` rather than its number. **Distinguishing "open a new PR" (`open_pr`) from "make the existing one ready" (`mark_pr_ready`) in the router is what was actually broken the first time this shipped** -- "set that to active" was misrouted into `open_pr`, which tried to create a second PR for a branch that already had one and got a real 422 from GitHub ("A pull request already exists"). Fixed by tightening both commands' descriptions to explicitly say new-vs-existing, not by adding special-case logic.
